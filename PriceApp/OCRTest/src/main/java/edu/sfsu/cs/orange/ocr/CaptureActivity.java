...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback,
  ...
  public static String retext1;

  public static String retext2;
  ...
  void handleOcrDecode(OcrResult ocrResult) {
    ...
    retext=ocrResult.getText();
    Log.i("1",retext);
    int length=retext.length();
    for(int i=1;i<length;i++){
      retext1=retext.substring(length-1-i,length-i);
      retext2=retext.substring(length-i,length-i+1);
      if(Pattern.matches("\\s",retext1)&&(!Pattern.matches("\\.",retext2))){
        string1=retext.substring(0,length-1-i);
        string2=retext.substring(length-i,length);
        break;
      }
    }
    ocrResultTextView.setText(retext);
    ...
    Intent ii=new Intent("edu.sfsu.cs.orange.ocr.ii");
    ii.putExtra("name",string1);
    ii.putExtra("price",string2);
//    Log.d("kangzhaoyaohahaha",string1);
